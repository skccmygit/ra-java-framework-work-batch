...
이 프로젝트는 Java(Spring Boot) 기반의 배치 처리 및 스케줄링 솔루션입니다. 전체 코드베이스를 분석한 결과, 계정 관리를 자동화하고 스케줄링을 통해 여러 작업을 처리하며, 다양한 데이터 처리 작업을 수행하도록 구성되어 있습니다. MySQL 또는 H2 데이터베이스를 사용하며, 로컬 환경에서는 Docker Compose로 실행할 수 있고 Gradle을 통해 빌드됩니다.
